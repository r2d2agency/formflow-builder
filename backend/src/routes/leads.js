...
router.get('/export/csv', async (req, res) => {
  try {
    const pool = req.app.locals.pool;
    const { form_id: formId, start_date, end_date, show_partial, search } = req.query;
    const showPartial = show_partial === 'true';
    const userId = req.user.id;
    const isAdmin = req.user.role === 'admin';

    let query = `
      SELECT l.id, l.data, l.source, l.ip_address, l.is_partial, l.created_at, f.name as form_name
      FROM leads l
      JOIN forms f ON l.form_id = f.id
    `;
    const params = [];
    ...
    const headers = ['ID', 'Formulário', 'Dados', 'Status', 'Fonte', 'IP', 'Data'];
    const rows = result.rows.map(row => [
      row.id,
      row.form_name,
      JSON.stringify(row.data),
      row.is_partial ? 'Incompleto' : 'Completo',
      row.source || '',
      row.ip_address || '',
      row.created_at,
    ]);
...
router.get('/export/excel', async (req, res) => {
  try {
    ...
    let query = `
      SELECT l.id, l.data, l.source, l.ip_address, l.is_partial, l.created_at, f.name as form_name, f.fields
      FROM leads l
      JOIN forms f ON l.form_id = f.id
    `;
    const params = [];
    ...
    // Apply text search filter in-memory (searches across all data fields)
    if (search && result.rows.length > 0) {
      const searchLower = search.toLowerCase();
      result.rows = result.rows.filter(row => {
        const dataStr = JSON.stringify(row.data).toLowerCase();
        const formName = (row.form_name || '').toLowerCase();
        return dataStr.includes(searchLower) || formName.includes(searchLower);
      });
    }

    if (result.rows.length === 0) {
      return res.status(404).json({ success: false, error: 'Nenhum lead encontrado com os filtros aplicados' });
    }
    ...
    const headers = ['ID', 'Formulário', ...fieldKeys, 'Status', 'Fonte', 'IP', 'Data'];
